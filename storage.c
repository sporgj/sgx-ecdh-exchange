              ...
              uint32_t            sealed_privkey_len)
{
    ...
    pubkey_str = nexus_alt64_encode(pubkey->bytes, sizeof(struct public_key));
              ...
              uint32_t           * sealed_privkey_len)
{
    ...
    memcpy(pubkey->bytes, pubkey_buf, sizeof(struct public_key));
...
{
    struct rk_initial * result = nexus_malloc(sizeof(struct rk_initial));
...
store_xchg_message(const char * filepath, struct rk_exchange * message)
{
    ...
    nonce_str = nexus_alt64_encode(message->nonce.bytes, sizeof(struct nonce));
    ciphertext_str = nexus_alt64_encode(message->ciphertext, message->ciphertext_len);
    ephemeral_pk_str
        = nexus_alt64_encode((uint8_t *)&message->ephemeral_pubkey, sizeof(struct public_key));
...
fetch_xchg_message(const char * filepath)
{
    ...
    memcpy(message->ephemeral_pubkey.bytes, pubkey_buf, sizeof(struct public_key));
    memcpy(message->nonce.bytes, nonce_buf, sizeof(struct nonce));
out:
